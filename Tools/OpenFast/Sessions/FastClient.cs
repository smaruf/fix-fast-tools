...
namespace OpenFAST.Sessions
{
    public sealed class FastClient
    {
        ...
        public void RegisterTemplatesFromXML(Stream xmlFileStream)
        {
            var templateLoader = new XmlMessageTemplateLoader { LoadTemplateIdFromAuxId = true };
            templateLoader.Load(xmlFileStream);

            _inboundRegistry = templateLoader.TemplateRegistry;
            _outboundRegistry = templateLoader.TemplateRegistry;
        }
