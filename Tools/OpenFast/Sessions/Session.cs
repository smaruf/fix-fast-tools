...
using OpenFAST.Sessions.Multicast;
    ...
    public sealed class Session : IErrorHandler
    {
                       ...
                       ITemplateRegistry outboundRegistry)
        {
            ...
            try
            {
                ...
                if (connection is not MulticastConnection)
                {
                    ...
                    () =>
                        {
                            while (_listening)
                            {
                                try
                                {
                                    ...
                                    //todo
                                    // if template is not identified then message is null.
                                    // in that case following code will call break and that's why out of thread.
                                    // which will not be desired
                                    if (message == null)
                                    {
                                        _listening = false;
                                        break;
