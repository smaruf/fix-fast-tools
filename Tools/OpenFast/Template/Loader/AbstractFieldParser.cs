...
namespace OpenFAST.Template.Loader
{
    public abstract class AbstractFieldParser : IFieldParser
    {
        ...
        #region Compatibility overloads

        // Some builds expect IFieldParser that accepts string element names. Provide
        // compatibility overloads that delegate to the XmlElement-based implementations
        // or perform a best-effort check.
        public virtual bool CanParse(string elementName, ParsingContext context)
        {
            return _parseableNodeNames.Any(t => t.Equals(elementName));
        }

        public Field Parse(string fieldNodeName, ParsingContext parent)
        {
            // Parsing from a raw string is not supported by this parser. Report error
            // via the error handler and return null to indicate failure.
            if (parent?.ErrorHandler != null)
            {
                parent.ErrorHandler.OnError(null, OpenFAST.Error.StaticError.InvalidXml,
                    "Parsing from string is not supported by this parser: {0}", fieldNodeName);
            }
            return null;
        }
