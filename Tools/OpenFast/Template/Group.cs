...
namespace OpenFAST.Template
{
    ...
    {
        // When true, suppress error logging for presence-map overlong / too many bits
        // This allows downstream sanitizers and defensive template absorbers to handle
        // vendor extensions without flooding error logs. Default true for backwards compatibility.
        public static bool LenientPresenceMapHandling { get; set; } = true;
                                                ...
                                                Context context)
        {
            ...
            if (pmap.IsOverlong)
            {
                ...
                {
                    // lenient mode: ignore and continue, but log a short diagnostic to console
                    try
                    {
                        Console.WriteLine($"[WARN] Presence map overlong for group {this.Name}: {pmap}");
                    }
                    catch
                    {
                                                  ...
                                                  BitVectorReader pmapReader, Context context)
        {
            ...
            if (pmapReader.HasMoreBitsSet)
            {
                ...
                {
                    // lenient mode: ignore extra bits and continue, but log a short diagnostic to console
                    try
                    {
                        Console.WriteLine($"[WARN] Presence map has extra bits for group {this.Name}; ignoring extras. Reader={pmapReader}");
                    }
                    catch
                    {
                        // swallow
