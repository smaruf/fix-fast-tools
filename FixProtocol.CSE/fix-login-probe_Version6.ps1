...
for($attempt=1; $attempt -le $maxAttempts; $attempt++) {
  ...
  Write-Host ("[TIME] Response time: {0} ms ({1:n3} s)" -f $elapsedMs, ($elapsedMs/1000.0))
  ...
  Write-Host ("[INFO] Reply 35={0}. Will increment seq and retry." -f $msgType)
  Write-Host $pretty
