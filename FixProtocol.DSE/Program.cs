...
using FixProtocol.DSE;

Console.WriteLine("==============================================");
Console.WriteLine("FIX Protocol Client/Server for DSE-BD");
...
static void RunServer(ILoggerFactory loggerFactory)
{
    ...
    logger.LogInformation("Starting FIX Server for DSE-BD...");

    // Create server configuration
    var settings = new SessionSettings();
    var sessionID = new SessionID("FIX.4.4", "DSE-BD", "CLIENT");
    var dictionary = new SettingsDictionary();
    
    dictionary.SetString("ConnectionType", "acceptor");
    dictionary.SetString("SocketAcceptPort", "5001");
    ...
    try
    {
        ...
        logger.LogInformation("Listening on port 5001");
...
{
    var logger = loggerFactory.CreateLogger<Program>();
    logger.LogInformation("Starting FIX Client for DSE-BD...");

    // Create client configuration
    var settings = new SessionSettings();
    var sessionID = new SessionID("FIX.4.4", "CLIENT", "DSE-BD");
    var dictionary = new SettingsDictionary();
    ...
    try
    {
        ...
        logger.LogInformation("Connecting to localhost:5001");
        logger.LogInformation("Session: {SessionID}", sessionID);
        ...
        {
            logger.LogInformation("Successfully connected to DSE-BD FIX Server!\n");
            ...
            {
                Console.WriteLine("\nOptions:");
                Console.WriteLine("1. Send test order (ACI stock)");
                ...
                {
                    case "1":
                        logger.LogInformation("Sending test order for ACI stock...");
                        application.SendNewOrder("ACI", "BUY", 100, 876.50m);
                        break;
...
static void SendCustomOrder(FixClient client, ILogger logger)
{
    try
    {
        Console.Write("Enter symbol (e.g., ACI, ACIFORMULA): ");
